...
| `public.ufficio_admin_users` | `user_id` | Delete for normal admins, block only the last active owner | Admin membership is account-bound, but the final active owner should not remove the last owner role through self-service | Checked server-side before deletion; last-owner protection remains enforced | Yes, via auth-user deletion cascade plus last-owner guard in the Edge Function | Last active owner may require support review |
