...
begin
  ...
  values (
    ...
    plan_input,
    coalesce(nullif(trim(source_input), ''), 'system'),
...
grant execute on function public.log_ufficio_public_event(text, uuid, text, jsonb, jsonb, text) to authenticated;
...
begin
  ...
  if promo_row.promo_kind = 'percent_discount' then
    ...
    target_plan_keys := to_jsonb(
      coalesce(
        promo_row.target_plan_keys,
        array['premium_monthly','premium_yearly']::text[]
      )
    );
...
exception
  when others then
    return jsonb_build_object(
      'ok', false,
      'message', coalesce(nullif(SQLERRM, ''), 'Could not redeem this code right now.')
    );
end;
$$;
